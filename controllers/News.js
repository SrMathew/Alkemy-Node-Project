...
const allNews = async (req, res, next) => {
    ...
    try {
        ...
        if (Object.keys(news).length === 0) {
            return setResponseWithError(res, statusCodes.BAD_REQUEST, statusMessages.BAD_REQUEST, 'error', news )
        } else {
...
const getNewsId = async (req, res, next) => {
    try {
        ...
        if (!news) {
           return setResponseWithError(res, statusCodes.BAD_REQUEST, statusMessages.INVALID_ID, 'error', news)
        }  else {
        setResponseWithOk(res, statusCodes.OK, statusMessages.SUCCESS, 'ok', news)  
        }
    } catch (error) {
...
const postNews = async (req, res, next) => {
    ...
    try {
        ...
        setResponseWithOk(res, statusCodes.OK, statusMessages.SUCCESS_CREATE, 'ok', newNews)
    } catch (error) {
        next(error)
    }
}
...
const updateNews = async (req, res, next) => {
    ...
    try {
        ...
        if (!exists) {
             return setResponseWithError(res, statusCodes.BAD_REQUEST, statusMessages.INVALID_ID)
        }

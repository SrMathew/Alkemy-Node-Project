...
const checkExistence = async (req, res, next) => {
  try {
    ...
    if (!organization) {
      ...
      setResponseWithOk(res, statusCodes.OK, statusMessages.SUCCESS, 'ok', organization)
    }
  } catch (error) {
      next(error)
  }
...
const addOrganization = async (req, res, next) => {
  ...
  try {
    ...
    setResponseWithOk(res, statusCodes.OK, statusMessages.SUCCESS, 'ok', newOrganization)
  } catch (error) {
      next(error);
    }
};

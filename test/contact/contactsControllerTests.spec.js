...
describe("Contact Controller methods", function () {
  beforeEach(function () {
    sinon.stub(contact,"findAll").returns({name:"dario",age:19,estudiante:true});
    sinon.stub(contact,"findByPk").returns({name:"dario",age:19,estudiante:true});
  ...
  it("should test if the getContacts method uses the stubs and return fake users ", async function () {
    req = {};
    res = {
  ...
  it("should test if the createContacts method uses the stubs and return a fake createdUser ", async function () {
    req = {body: {name:"Link",age:22,adventure:true,password:"1234",email: 'email@test.test'}};
    res = {
      status:(status)=>status,
      send:(respuesta)=>{return{respuesta}}
    };
